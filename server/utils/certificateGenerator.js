...
const generateCertificate = async (studentName, courseName, instructorName, completionDate, template = 'classic') => {
    ...
    try {
        ...
        // Add executablePath for production environments if needed
        ...
        // Set content with timeout and wait for fonts to load
        ...
        return await generateFallbackCertificate(studentName, courseName, instructorName, completionDate, template);
...
// Fallback certificate generation using simple HTML to PDF conversion
const generateFallbackCertificate = async (studentName, courseName, instructorName, completionDate, template) => {
    try {
        console.log('Using fallback certificate generation method');
        
        // Simple HTML certificate without external dependencies
        const fallbackHTML = generateFallbackHTML(studentName, courseName, instructorName, completionDate, template);
        
        // Try to use a simpler PDF generation method
        const puppeteerSimple = require('puppeteer');
        let browser = null;
        
        try {
            browser = await puppeteerSimple.launch({
                headless: true,
        ...
        // Final fallback: Return a basic certificate buffer
        return generateBasicCertificateBuffer(studentName, courseName, instructorName, completionDate);
    }
};

// Generate a basic certificate as a last resort
const generateBasicCertificateBuffer = (studentName, courseName, instructorName, completionDate) => {
...
EduMe Learning Platform
    `;
    
    return Buffer.from(certificateText, 'utf8');
};

// Simple fallback HTML template
const generateFallbackHTML = (studentName, courseName, instructorName, completionDate, template) => {
    return `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        <div class="certificate">
            ...
            <div class="details">
                ...
                <p>Date: ${new Date(completionDate).toLocaleDateString()}</p>
                <p>EduMe Learning Platform</p>
            </div>
        </div>
    </body>
    </html>
